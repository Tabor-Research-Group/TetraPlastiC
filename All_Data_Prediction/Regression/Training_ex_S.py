...
def load_data(index):
    ...
    df_train = pd.read_csv(f'S/{index[0]}/{index[1]}/train.dat', sep='\t')

    df_test = pd.read_csv(f'S/{index[0]}/{index[1]}/test.dat', sep='\t')
...
def read_model_files(index):
    ...
    with open(f'S/{index[0]}/{index[1]}/Models/top0100_D002', 'r') as f:

        lines_model = f.readlines()

    with open(f'S/{index[0]}/{index[1]}/SIS_subspaces/Uspace.expressions', 'r') as f:
...
np.save('all_model_Y_train_S.npy',all_model_Y_train)

np.save('all_model_Y_test_S.npy',all_model_Y_test)
