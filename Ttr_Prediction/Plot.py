...
def plot_feature_scores(selected_feature, score_stacking):
    ...
    selected_feature_modified = np.array([r'$\sigma_\theta$', '$\it{Br}$', '$\it{C_4H_9}$', '$\it{CH_2Cl}$', '$\it{CH_2F}$', '$\it{CH_2NH2}$',
       '$\it{CHClOH}$','$\it{CHFOH}$', '$\it{CHO}$', '$\it{COCl}$', '$\it{COOH}$', 'Center of Mass', '$\it{Cl}$',
       '$E_{CT}$', '$E_{dispersion}$', '$\it{F}$',
       'Interaction Energy', '$Max_{acc}$', '$\it{NH_2}$', '$\it{NO_2}$', '$\mu_R$',
    ...
    ax.legend()
    plt.title('Selected Feature Ratio from Transition Temperature Prediction', fontsize=12)
    plt.savefig('Prediction_Ttr.png')
